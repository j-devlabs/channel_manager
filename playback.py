...
def start_channel(cfg):
    ...
    # Open log files
    stdout_log = cfg.get("log_stdout", os.path.join(out_dir, f"{cid}.out.log"))
    stderr_log = cfg.get("log_stderr", os.path.join(out_dir, f"{cid}.err.log"))
...
# Stop a single channel by reading its PID file and sending SIGTERM
...
# (Optional) functions for per-channel playlist & guide generation